...
app.get('/PesertaGameIndividuDiterima', function(req,res){
    ...
    let sql = "SELECT * FROM peserta_individu pi inner join game_individu gi on gi.idgame = pi.idpeserta WHERE status = 1 or status= 3 ";
    let query = conn.query(sql,function(err,result){
        if (err) throw err;
        res.send(JSON.stringify({
            "status" : 200,
...
app.get('/PesertaGameIndividuPending', function(req,res){
    console.log('Menerima GET request /PesertaGameIndividu-Pending');
...
app.get('/teamTerima', function(req,res){
    console.log('Menerima GET request /Team-terima');
    let sql = "SELECT * FROM team t inner join peserta_kelompok pk on t.idteam = pk.team_idteam inner join game_kelompok gk on t.game_kelompok_idgame=gk.idgame where status =1 or status =3 group by nama_team  ;";
    let query = conn.query(sql,function(err,result){
        if (err) throw err;
        res.send(JSON.stringify({
            "status" : 200,
...
app.get('/TeamDiterima', function(req,res){
    ...
    let sql = "SELECT * FROM team t inner join peserta_kelompok pk on t.idteam = pk.team_idteam inner join game_kelompok gk on t.game_kelompok_idgame = gk.idgame WHERE status = 1 OR t.status = 3 group by nama_Team; ";
    let query = conn.query(sql,function(err,result){
        if (err) throw err;
        res.send(JSON.stringify({
            "status" : 200,
...
app.get('/bracketIndivOngoing', function(req, res) {
    console.log('Menerima GET request /bracketIndivOngoing');
    let sql = "SELECT p1.nama AS peserta1, p2.nama AS peserta2, b.date, b.status,b.idbracketIndiv FROM bracket_individu b JOIN peserta_individu p1 ON b.idpeserta = p1.idpeserta JOIN peserta_individu p2 ON b.idpeserta1 = p2.idpeserta WHERE b.idpeserta IS NOT NULL AND b.idpeserta1 IS NOT NULL AND b.status = 0;";
    let query = conn.query(sql, function(err, result) {
        if (err) throw err;
        res.send(JSON.stringify({
            "status": 200,
...
app.post('/updateBracketIndividu', function(req, res) {
    console.log('Menerima POST request /updateBracketIndividu');
    let data = { idbracketIndiv: req.body.idbracketIndiv };
    let sql = "UPDATE bracket_individu SET `status` = '1' WHERE `idbracketIndiv` = " + data.idbracketIndiv;
...
app.post('/updateBracketTeam', function(req, res) {
    console.log('Menerima POST request /updateWinLosePeserta_individu');
    let data = { idBracketTeam: req.body.idBracketTeam };
    let sql = "UPDATE bracket_team SET `status` = '1' WHERE `idBracketTeam` = " + data.idBracketTeam;
...
app.get('/bracketTeamOngoing', function(req,res){
    console.log('Menerima GET request /bracketTeamOngoing');
    let sql = "SELECT t1.nama_team AS team1, t2.nama_team AS team2, bt.date ,bt.komentar,bt.status,bt.idBracketTeam    FROM bracket_team bt    JOIN team t1 ON bt.team_idteam = t1.idteam   JOIN team t2 ON bt.team_idteam1 = t2.idteam where bt.status = 0;";
    let query = conn.query(sql,function(err,result){
        if (err) throw err;
        res.send(JSON.stringify({
            "status" : 200,
